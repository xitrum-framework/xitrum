...
val url    = "http://svn.apache.org/repos/asf/httpd/httpd/trunk/docs/conf/mime.types"
...
for (line <- lines) {
  if (!line.startsWith("#")) {
    ...
    if (size >= 2) {
      ...
      for (i <- 1 to size - 1) {
        val extension = tokens(i)
        buffer.append((extension, mime))
...
val format = "%" + maxLength + "s\" -> \"%s\","
for ((extension, mime) <- buffer) println(format.format("\"" + extension, mime))
*/
